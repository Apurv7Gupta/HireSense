...
- **Google Gemini API** - Google's state-of-the-art large language model that powers the AI analysis in this application. Specifically, we use the `gemini-2.5-flash` model, which offers an excellent balance of speed and accuracy.
...
HireSense uses a modern, scalable architecture that combines the best of web and AI technologies:
...
This architecture ensures that HireSense can handle multiple file formats, process large batches of resumes efficiently, and provide consistent, high-quality analysis results.
...
5.  Go to the "Environment" tab and add a **New Environment Variable**:
    - **Key**: `GEMINI_API_KEY`
    - **Value**: Pase your secret Gemini API key here.
